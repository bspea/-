...
/**
 * Servlet implementation class FilesCoverServlet
 */
@WebServlet("/fileCover.me")
public class FilesCoverServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public FilesCoverServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			dbRoot = dbRoot+"/resources/fileupload_thumbnail/"; // dbRoot = /teamace + /resources/fileupload_thumbnail/
			
			
			// 파일이 실제로 저장될 경로
			String savePath = root + "/fileupload_thumbnail/";
			...
			mb.setCoverImage(dbRoot);
			
			int result = new MemberService().coverImageUpdate(mNo, mb);
			response.setContentType("text/html; charset=UTF-8"); 
		
			if(result > 0) {
				response.getWriter().print(dbRoot);
				loginUser.setCoverImage(dbRoot);
			}else {
